...
/**
 * The mark: a ring laid onto a single line, which is exactly what the notation
 * does. The line carries the second brand colour so the two halves of the idea
 * — the structure and the one line it is written on — stay told apart at 16 px.
 *
 ...
 *
 * The mark's orange reaches about 3.6:1 on white, short of what text needs, so
...
export function Wordmark(props: WordmarkProps) {
  ...
  return (
    <span className={className ? `wordmark ${className}` : 'wordmark'}>
      <span className="wordmark__lead">smiles</span>
