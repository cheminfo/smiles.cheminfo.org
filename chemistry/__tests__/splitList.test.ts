...
import { listStructures, parseList, splitList } from '../splitList.ts';
...
test('listStructures drops the names', () => {
  expect(listStructures('CCO ethanol\nc1ccccc1 benzene')).toStrictEqual([
    'CCO',
    'c1ccccc1',
  ]);
});
...
test('a header saying SMILES breaks the tie with a formula column', () => {
  const { entries, layout } = parseList(
